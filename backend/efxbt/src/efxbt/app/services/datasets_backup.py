...
class TradeBookDetail(BaseModel):
    """Detailed information about a trade book including pair-date inventory."""
    ...
    total_volume: float
    root_path: str
    pair_dates: list[dict[str, str | int | float]]
...
class TradeBookHealthResponse(BaseModel):
    ...
    trade_stats: list[dict[str, str | bool | int | float | None]]
...
class TradeBooksService:
    ...
    def get_tradebook_pairs(self, book_name: str) -> list[str]:
        """Get list of pairs in a trade book.
        ...
        trade_stats = [
            {
                ...
                "total_volume": ts.total_volume,
                "avg_trade_size": ts.avg_trade_size,
            }
