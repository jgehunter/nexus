...
_RISK_KPIS = [
    KPIDefinition(
        ...
        name="Peak Position",
        category="risk",
        description="The largest absolute inventory position held at any point during the simulation. Higher values indicate more risk exposure.",
        formula="max(|net_position|) across all timestamps",
        unit="Base CCY",
    ...
    KPIDefinition(
        ...
        name="Position P95",
        category="risk",
        description="The 95th percentile of absolute inventory. The position was below this level 95% of the time.",
        formula="percentile(|position|, 95)",
        unit="Base CCY",
        interpretation="lower_better",
    ),
    KPIDefinition(
        ...
        name="Position P99",
        category="risk",
        description="The 99th percentile of absolute inventory. Captures tail risk from extreme positions.",
        formula="percentile(|position|, 99)",
        unit="Base CCY",
        interpretation="lower_better",
    ),
    KPIDefinition(
    ...
    KPIDefinition(
        id="time_above_risk_band_pct",
        name="Time Above Band",
        category="risk",
        description="Percentage of time the absolute position exceeded the risk band threshold. Higher values indicate more time at elevated risk.",
        formula="time(|position| > threshold) / total_time * 100",
        unit="%",
        interpretation="lower_better",
    ),
    KPIDefinition(
...
_FRONTIER_KPIS = [
    ...
    KPIDefinition(
        ...
        name="Risk Score",
        category="frontier",
        description="Composite score combining P99 inventory and time above band. Used for efficient frontier analysis.",
        formula="normalized(p99) + normalized(time_above_band)",
        unit="score",
        interpretation="lower_better",
    ),
    KPIDefinition(
